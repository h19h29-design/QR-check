...
function verifyAdmin_(payload) {
  ...
  if (adminToken && tokenHash && hashToken_(adminToken, 'admin') === tokenHash) {
...
function verifyDesktop_(payload) {
  ...
  if (!syncKey || hashToken_(syncKey, 'desktop') !== expected) {
...
function generateAdminTokenForSetup_(payload) {
  const token = payload && payload.adminToken ? payload.adminToken : Utilities.getUuid() + Utilities.getUuid();
  setSetting_('admin_token_hash', hashToken_(token, 'admin'));
  return token;
}

function generateSyncKeyForSetup_(payload) {
  const key = payload && payload.syncKey ? payload.syncKey : Utilities.getUuid() + Utilities.getUuid();
