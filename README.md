...
> 신규 학교 자동 설치는 테스트 계정에서 실제 Google 자원 생성과 제출까지 검증했습니다. OAuth 앱과 브랜딩은 게시됐지만 데이터 액세스 인증 전이라 미검증 경고·사용자 한도·학교 Workspace 정책 제한이 남아 있습니다.

## 현재 상태

2026-09-12 재확인: 외부 프로덕션·브랜딩 인증은 완료됐지만 **Google 민감 권한 심사는 아직 제출되지 않았습니다.** 범위 근거와 실제 데모 영상 링크가 필요합니다. 이번 추가 테스트 학교는 Google 권한 승인 대기이며, 아래 2026-09-11의 완료 이력과 별개입니다. [최신 준비 상태와 남은 조건](docs/review/PUBLIC_READINESS_20260912.md)을 참고하세요. 현재는 시범 제공 단계로, 모든 학교에서 제한 없이 사용할 수 있는 상태는 아닙니다.

2026-09-11 기준으로 런타임 `0.1.1` 신규 설치를 다시 수행해 학교 소유 Drive 폴더·Sheet·Apps Script 프로젝트 생성, 웹앱 배포, 소유자 초기 설정, 담당자 추가, 정상·이상 QR 제출, 관리자 조회·상세·확인을 확인했습니다. PNG 첨부는 공개 제출 API에서 학교의 비공개 Drive 저장과 관리자 링크 열람까지 확인했으며, 실물 휴대전화의 파일 선택 UI는 별도 검증 대상입니다.

| 항목 | 상태 |
|---|---|
| 홈페이지 | `https://qr-safe.h19h19.com` HTTPS, GitHub Pages, 게시 런타임 `0.1.1` 확인 |
| 신규 학교 설치 E2E | 테스트 사용자 계정에서 `0.1.1` Google 자원 생성 → 초기 설정 → 정상·이상 QR 제출 → 관리자 확인 완료 |
| 첨부 경로 | PNG 1건을 공개 제출 API로 전송해 학교 Drive 저장·관리자 상세 링크 열람 확인 |
| 데이터 소유 | 학교 계정의 Sheets·Drive·Apps Script에 저장. 중앙 서버에 학교 기록·첨부·비밀키를 저장하지 않음 |
| 웹 관리자 | 기록 조회·상세·확인, 장소·QR 관리, 담당자·당직자 관리 제공 |
| 설치센터 OAuth | 외부 프로덕션·브랜딩 게시 완료, 데이터 액세스 미검증 |
| 일반 학교 공개 | Google 계정 접근 가능. 미검증 사용자 한도 100명·경고 화면·Workspace 관리자 차단 가능 |
| 추가 검증 필요 | OAuth 민감 범위 데이터 액세스 인증, 교육용 Workspace 정책, 실물 휴대전화·인쇄 QR, 브라우저 파일 선택기, 업데이트·롤백 라이브 검증 |
