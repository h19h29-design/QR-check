...
CHECKLISTS = [
    ...
    {"key": "class", "name": "교무실", "order": 4},         # 교무실 추가
    {"key": "help", "name": "학습도움실", "order": 5}       # 학습도움실 추가
]

RESPONSIBLE_PERSONS = {
    "facility": ["황정우", "백이선", "문관택"],
    "admin": ["김선미", "전은주", "최정순", "문상희", "이민정"],
    "print": ["황정우", "백이선", "문관택"]
    "class": ["변혜원", "이영호","주 이","이미진","문수진","이경미","안지은","권효진","최윤서","김이경","경지현","윤지영","최수화"]     # 교무실 담당자 추가
    "help": ["장희경", "김경화","남수연","김정훈"]                 # 학습도움실 담당자 추가
}
...
def get_latest_by_role(room):
    ...
    if duty and (not responsible or duty['id'] != responsible['id']):
        result.append(duty)
    return result
...
def home():
    ...
    # 실별 최신 점검(담당/당직자) 요약 - 실 순서대로 정렬
    room_summaries = []

    # 모든 실별로 데이터 가져오기
    for checklist in sorted(CHECKLISTS, key=lambda x: x['order']):
        latest = get_latest_by_role(checklist['key'])
        room_info = {
    ...
    return render_template('home.html', room_summaries=room_summaries)
...
def admin():
    ...
    if start_date and end_date:
        date_condition = f"WHERE timestamp BETWEEN '{start_date} 00:00:00' AND '{end_date} 23:59:59'"
...
def export_excel():
    ...
    for checklist in CHECKLISTS:
        ...
        for record in records:
            ...
            all_records.append({
                ...
                # 이상 유무 판단
                '이상유무': any(record[field] == '이상유' or record[field] == '이상 유'
                             for field in ['document', 'cleaning', 'lighting', 'fire', 'door'])
            })
    conn.close()
    ...
    for row_num, value in enumerate(df['이상유무'], start=1):
        if not value:  # 이상 없음(False)일 때만 동그라미 표시
...
def generate_qr():
 ...
 # 모든 실(점검표) 테이블 전체를 업데이트
@app.route('/admin/verify_all', methods=['POST'])
def verify_all_records():
    ...
    for checklist in CHECKLISTS:
        c.execute(f'''
            UPDATE inspections_{checklist["key"]}
            SET admin_verified = 1,
                verification_time = ?
